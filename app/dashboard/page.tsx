...
import {
  Zap, Flame, CheckCircle2, Circle, ArrowRight,
  Map, Briefcase, FolderGit2, Activity, Star, Target
} from "lucide-react";
...
export default function DashboardPage() {
  const { stats, nodeProgress, projectProgress, achievements, dailyTasks } = useProgressStore();
  ...
  const completedProjects = Object.values(projectProgress).filter((p) => p.status === "completed").length;
  ...
  return (
    <div className="space-y-6 max-w-7xl">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <h2 className="text-xl font-semibold text-zinc-100">Hallo! Zeit zum Lernen 👋</h2>
        <p className="text-sm text-zinc-500 mt-0.5">
          Level {level} · {stats.streak} Tage in Folge · {stats.totalHoursStudied} Stunden insgesamt
        </p>
      </motion.div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { icon: Zap, label: "XP", value: stats.xp.toLocaleString("de-DE"), sub: `Level ${level}`, color: "text-yellow-400", bg: "bg-yellow-500/10" },
          { icon: Flame, label: "Serie", value: `${stats.streak} Tage`, sub: "in Folge", color: "text-orange-400", bg: "bg-orange-500/10" },
          { icon: CheckCircle2, label: "Abgeschlossen", value: `${completedNodes}/${totalNodes}`, sub: "Roadmap-Nodes", color: "text-emerald-400", bg: "bg-emerald-500/10" },
          { icon: Star, label: "Abzeichen", value: achievements.length.toString(), sub: "Errungenschaften", color: "text-violet-400", bg: "bg-violet-500/10" },
        ].map((stat, i) => {
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm">XP-Fortschritt — Level {level}</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-zinc-500">Level {level}</span>
            <span className="text-xs text-zinc-400">{Math.round(xpProgress)}%</span>
            <span className="text-xs text-zinc-500">Level {level + 1}</span>
          </div>
          <Progress value={xpProgress} className="h-2.5" indicatorClassName="bg-gradient-to-r from-blue-500 via-violet-500 to-purple-500" />
          <div className="flex justify-between mt-1.5">
            <span className="text-[10px] text-zinc-600">{currentLevelXp} XP</span>
            <span className="text-[10px] text-zinc-500">{stats.xp} / {nextLevelXp} XP</span>
            <span className="text-[10px] text-zinc-600">{nextLevelXp} XP</span>
          </div>

          <div className="mt-4 grid grid-cols-3 gap-3">
            <div className="text-center p-3 rounded-lg bg-zinc-900">
              <div className="text-lg font-bold text-blue-400">{roadmapPercent}%</div>
              <div className="text-[10px] text-zinc-500">Roadmap</div>
            </div>
            <div className="text-center p-3 rounded-lg bg-zinc-900">
              <div className="text-lg font-bold text-emerald-400">{completedProjects}/{totalProjects}</div>
              <div className="text-[10px] text-zinc-500">Projekte</div>
            </div>
            <div className="text-center p-3 rounded-lg bg-zinc-900">
              <div className="text-lg font-bold text-orange-400">{dailyCompleted}/{dailyTotal}</div>
      ...
      <Card>
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="blue">{dailyCompleted}/{dailyTotal}</Badge>
          </div>
        </CardHeader>
        <CardContent>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-zinc-300">Roadmap-Sektionen</h3>
          <Button variant="ghost" size="sm" asChild>
            <Link href="/dashboard/roadmap">Gesamte Roadmap <ArrowRight className="w-3.5 h-3.5 ml-1" /></Link>
          </Button>
        </div>
                  ...
                  <Card className="hover:border-zinc-700 transition-colors cursor-pointer">
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-xs text-zinc-500">{sectionCompleted}/{section.nodes.length}</span>
                      </div>
                      <Progress value={sectionPct} className="h-1" />
                      <div className="flex items-center justify-between mt-1.5">
                        <span className="text-[10px] text-zinc-600">{sectionPct}% abgeschlossen</span>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { icon: Map, label: "Roadmap", href: "/dashboard/roadmap", color: "text-blue-400", bg: "bg-blue-500/10" },
          { icon: Briefcase, label: "Karriere", href: "/dashboard/karriere", color: "text-violet-400", bg: "bg-violet-500/10" },
          { icon: FolderGit2, label: "Projekte", href: "/dashboard/projekte", color: "text-emerald-400", bg: "bg-emerald-500/10" },
          { icon: Activity, label: "DevOps", href: "/dashboard/devops", color: "text-orange-400", bg: "bg-orange-500/10" },
        ].map((item) => {
...
function DailyTaskRow({ task }: { task: { id: string; title: string; xpReward: number; completed: boolean } }) {
